...
public static class CsvWriter
{
    ...
    public static async Task WriteAsync(string path, IReadOnlyList<TypeMetrics> types,
        IReadOnlyList<MemberMetrics> members, CancellationToken cancellationToken = default)
    {
        ...
        {
            var typeLine = $"Type,{Escape(t.Project)},{Escape(t.Namespace)},{Escape(t.Type)},,{t.MaintainabilityIndex},{t.CyclomaticComplexity},{t.DepthOfInheritance},{t.ClassCoupling},{t.LinesOfSource},{t.LinesOfExecutable}";
            ...
            {
                var memberLine = $"Member,{Escape(m.Project)},{Escape(m.Namespace)},{Escape(m.Type)},{Escape(m.Member)},{m.MaintainabilityIndex},{m.CyclomaticComplexity},0,0,{m.LinesOfSource},{m.LinesOfExecutable}";
                await writer.WriteLineAsync(memberLine.AsMemory(), cancellationToken);
            }
        }
    }
